...
def directed_homophilic_barabasi_albert_graph(N, kmin, density, minority_fraction, h_mm, h_MM, gamma_m, gamma_M, seed=None):
    ...
    activity_distribution[[n for n in G.nodes()
                           if n not in minority_nodes]] = power_law_distribution(k_min_activity, gamma_M, N - minority)
...
def _pick_targets(G, source, target_list, homophily_dic, kmin):
    ...
    prob = np.array(
        [homophily_dic[(label_source, G.node[target][label])] * (G.in_degree(target) + 1) for target in target_list])
    prob += smooth
    prob = prob / prob.sum()
    return np.random.choice(a=target_list, size=kmin, replace=False, p=prob)


def estimate_homophily_empirical(graph, fm=None, EMM=None, EMm=None, EmM=None, Emm=None, betaM=None, betam=None, verbose=False):
    ...
    for h_mm_ in np.arange(0, 1.01, 0.01):
        for h_MM_ in np.arange(0, 1.01, 0.01):
            ...
            if betaM is None:
                try:
                    beta_M = float(fM * h_MM_analytical) / (
            ...
            if betam is None:
                try:
                    beta_m = float(fm * h_mm_analytical) / ((h_mm_analytical * cm) + (h_mM_analytical * (2 - cm))) + float(
                        fM * h_Mm_analytical) / ((h_Mm_analytical * cm) + (h_MM_analytical * (2 - cm)))
                except RuntimeWarning:
                    if verbose:
                        print('break 1')
