...
def DBA(N, fm, d, plo_M, plo_m, verbose=False, seed=None):
    '''
    ...
    G = nx.DiGraph()
    G.graph = {'name':'DBA', 'label':CLASS, 'groups': GROUPS}
    ...
    while G.number_of_edges() < E:
        ...
        ns = nodes[source]
        target = _pick_target(source, N, labels, indegrees)
        nt = nodes[target]
...
def _pick_target(source, N, labels, indegrees):
    '''
